document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Clear existing dropdown options (keep the first default option)
  ...
  // Make unregisterParticipant function global so it can be called from onclick handlers
  window.unregisterParticipant = async function(activityName, email) {
    ...
    try {
      ...
      if (response.ok) {
        messageDiv.textContent = result.message;
        messageDiv.className = "success";
        // Refresh the activities list to show the removed participant
        fetchActivities();
      } else {
        messageDiv.textContent = result.detail || "An error occurred";
        messageDiv.className = "error";
